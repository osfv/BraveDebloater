...
- Restore now rejects a backup whose `policyKind` does not match its recorded path (for example a registry key path with a JSON-file kind), rejects unknown kinds, and stops early without root when restoring the Linux default policy file.
...
- `-PruneBackupsOlderThanDays` and `-KeepLatestBackups` also remove the profile `Preferences` copies that belong only to the pruned backups, and previews list them.
...
- `scripts/Test-LatestPolicyTemplates.ps1` now accepts a newer `latest` Brave template with a warning instead of failing CI on every Brave release (pass `-RequireVersionMatch` for release checks), and validates every manifest value against the ADMX definition so `0`/`1` are only used for boolean policies.
